...
def setup_logger():
    ...
    handler.setFormatter(formatter)  # 设置文件处理器的格式
    logger_sim.addHandler(handler)   # 将文件处理器添加到日志记录器

    # 创建控制台处理器，将日志输出到控制台  
    console_handler = logging.StreamHandler()
    console_handler.setFormatter(formatter)  # 设置控制台处理器的格式
    logger_sim.addHandler(console_handler)   # 将控制台处理器添加到日志记录器  

    _logger_instance = logger_sim  # 保存全局日志记录器实例  
...
class PiStateCali:
    def __init__(self,
        ...
        # 运行sumo模拟
        self.sumo = AAO_Singlemulation(
            taskname='aao_stateidentify',
            model=model,
            state_flag=True,
            freq=self.frequency, 
    ...
    def run(self, in_state: str, out_state: str, inf_args: dict, goal_mlss, goal_tn):
        ...
        Args:  
            in_state (str): 输入状态文件的路径格式。  
            out_state (str): 输出状态文件的路径格式。  
            inf_args (dict): 输入参数字典。  
            goal_mlss (list): 目标MLSS值列表。  
            goal_tn (float): 目标TN值。  
    ...
    def run_Sim(self, state, n, qpump, inf_tn):
        ...
        Args:  
            state (str): 状态文件模板 out_path / (f"{delta_mlss}_{delta_tn}_" + "state{}_init.xml")
            n (int): 模拟步数。  
            qpump (list): 排泥。  
            inf_tn (float): 进水TN值。  

        Returns:  
            dict: 模拟结果,包括TN和MLSS等值。 
        """  
    ...
    def set_mlss_TN(self, goal_mlss: list, goal_tn: float):
        ...
        parameter:
            ...
            goal_tn - 目标TN值
        """
        delta_tn = self.delta_tn  # TN的容许误差  
        delta_mlss = [self.delta_mlss * goal_mlss[i] for i in range(WATER_LINE)]  # MLSS的容许误差

        # 初始化绘图数据
        draw_data = {
            "mlss": [[] for _ in range(WATER_LINE)],
            "qpump": [[] for _ in range(WATER_LINE)], 
            "eff_tn": [], 
            "inf_tn": [], 
            "t": []
        }
        t = -1
        sum_delta_mlss = [0.0 for _ in range(WATER_LINE)]  # MLSS的积分和  
        sum_delta_tn = 0.0  # TN的积分和
        inf_tn = None  # 入水tn
        qpump = [None for _ in range(WATER_LINE)]  # 排泥量
        method = self.method  # 积分抗饱和策略
        eff_tn = 0  # 出水tn
        mlss = [0,0,0,0]  # 出水mlss
        while (
            t <= 0
            ...
            plt.close(fig)
            fig.savefig(os.path.join(self.out_path, "tn_output.png"))  # 保存TN输出图
...
class HumanStateCali:
    ...
    该类实现了对模型的初始状态进行校准的功能,校准处理污水处理过程中的MLSS和TN值。  

    Attributes:  
        model (list): 模拟的模型文件列表。  
        tf: 用于处理状态的对象。  
        frequency (int): 模拟运行的频率。  
        timeout (int): 模拟超时时间（秒）。  
        abs_mlss (float): MLSS的绝对容许误差。  
        delta_tn (float): TN的绝对容许误差。  
        sub_randstate (str): 随机状态文件的路径格式。  
        MLSS (list): 记录四个MLSS的名称。  
        is_save (bool): 标记是否保存状态。  
        map_table (dict): 映射表，用于将输出变量名称映射到统一名称。  
        balance_point (list): 每个水线的污泥平衡点。  
        aao_mlss (list): 存储AAO的MLSS值。  
    """  
    def __init__(self, model, tf,
        ...
        self.balance_point = [None]*WATER_LINE  # 初始化每个水线的污泥平衡点  
        self.aao_mlss = []  # 初始化AAO的MLSS值 
    ...
    def find_Balance(self, task_index, inf_tn=None):
        ...
        让mlss不变的排泥量, 更大的排泥量将导致mlss减小, 更小的排泥量将导致mlss增加
         ...
         Args:  
            task_index (int): 当前任务的索引。  
            inf_tn (float, optional): 输入TN值,默认为None。  
    ...
    def set_Mlss(self, goal_mlss, task_index, inf_tn, all_balance_point):
        ...
        Args:  
            goal_mlss (float): 目标MLSS值。  
            task_index (int): 当前任务的索引。  
            inf_tn (float): 输入TN值。  
            all_balance_point (list): 所有水线的平衡点列表。  
    ...
    def effluent_tn(self, aao1_output, aao2_output, aao3_output, aao4_output):
        ...
        该方法把相同的key是汇合到一个effluent的。  

        Args:  
            aao1_output (dict): 第一个AAO的输出结果。  
            aao2_output (dict): 第二个AAO的输出结果。  
            aao3_output (dict): 第三个AAO的输出结果。  
            aao4_output (dict): 第四个AAO的输出结果。  
    ...
    def set_Eff_Tn(self, goal_eff_tn, eff_tn, balance_point, inf_tn, eff_mlss):
        ...
        Args:  
            goal_eff_tn (float): 目标出水TN值。  
            eff_tn (float): 当前模拟出水TN值。  
            balance_point (list): 平衡时排泥量。  
            inf_tn (float): 输入TN值。  
            eff_mlss (list): 当前MLSS值列表。  

        Returns:  
            tuple: (inf_tn, eff_tn, eff_mlss) 最终的输入TN,输出TN和MLSS值。  
        """
        #logger = setup_logger\(\)
    ...
    def run_Sim(self, n, task_index, flow6_qpump=None,inf_tn=None, inf_cod=None,result_dic=None):
        ...
        Args:  
            n (int): 表示n次单频率运行。  
            task_index (int): 当前任务的索引。  
            flow6_qpump (float, optional): 调整的排泥量,默认为None。  
            inf_tn (float, optional): 输入TN值,默认为None。  
            inf_cod (float, optional): 输入COD值,默认为None。  
            result_dic (dict, optional): 用于存储结果的字典,默认为None。  
        """
        # 任务并发数为1
    ...
    def new_single_process_rum_sim(self, n, task_index, flow6_qpump=None,inf_tn=None, inf_cod=None): 
        ...
        result_dic = mp.Manager().dict()  # 使用多进程管理器创建结果字典
        process = mp.Process(target=self.run_Sim, args=(n, task_index, flow6_qpump, inf_tn,inf_cod,result_dic))  # 创建进程
        process.start()  # 启动进程
        process.join()  # 等待进程结束 
    ...
    def get_Initial_State(self, task_index, all_res, all_balance_point, inf_tn=None):
        ...
        Args:  
            task_index (int): 当前任务的索引。  
            all_res (list): 存储所有任务结果的列表。  
            all_balance_point (list): 存储所有任务平衡点的列表。  
            inf_tn (float, optional): 输入TN值,默认为None。  
        """
        logger = setup_logger()
    ...
    def get_commands(self, n, commands_index, flow_qpump_param=None, inf_tn_1=None, inf_cod_1=None):
        ...
        Args:  
            n (int): 表示单词模拟的次数。  
            commands_index (int): 选择要生成的命令组的索引。  
            flow_qpump_param (float, optional): 调整的排泥量,默认为None。  
            inf_tn_1 (float, optional): 输入TN值,默认为None。  
            inf_cod_1 (float, optional): 输入COD值,默认为None。  
    ...
    def get_variables(self,variables_index):
        ...
        Args:  
            variables_index (int): 变量组的索引，范围从0到3。  
    ...
    def execute_task(self, out_state_file_format: str, inf_args, goal_mlss, goal_tn):
        ...
        Args:  
            out_state_file_format (str): 存储校准后的状态文件格式。  
            inf_args (dict): 运行参数字典。  
            goal_mlss (list): 每条AAO的MLSS目标值。  
            goal_tn (float): 总出水目标TN值。  
        """  
        logger = setup_logger()
...
class AAO_State_Identify:
    ...
    def __init__(self, model,  
        ...
        self.cali0 = HumanStateCali(model=model, tf=self.tf, init_state_file_format=init_state_file_format,
                 timeout=timeout, freq=freq, abs_mlss=delta_mlss, delta_tn=delta_tn)  #老方法
        self.cali1 = PiStateCali(mlss_Kp, mlss_Ki, mlss_imax, tn_Kp, tn_Ki, tn_imax,
                        model, freq,
                        delta_mlss, delta_tn, method=a_saturation_method)  #新方法
    ...
    def set_attribute(self, attr_name, value):
        ...
        if isinstance(attr_name, str) and attr_name.isidentifier():
            setattr(self, attr_name, value)  # 使用内置函数设置属性 
        else:
    ...
    def do_state_identify(self, tag:str, ct_start: datetime.datetime, ct_end: datetime.datetime,
        ...
        :param tag: str, 场景标签，用于区分不同的状态识别场景。  
        :param ct_start: datetime.datetime, 开始时间，任务执行的起始时间。  
        :param ct_end: datetime.datetime, 结束时间，任务执行的结束时间。  
        :param out_state_file_format: str | None, 输出的状态文件格式。如果为 None,则根据模式生成默认文件名。  
    ...
    def get_db_data_on_ct(self, ct_start, ct_end=None):
        ...
        :param ct_start: datetime.datetime, 查询的开始时间。  
        :param ct_end: datetime.datetime, 查询的结束时间。
        :returns: tuple, 包含三个元素：  
            - inf_record: dict , 清洗数据的记录。  
            - eff_dis_tn: float , 出水 TN 值。  
            - inf_records: list , 所有清洗记录的列表。  
        """ 
        # 连接数据库
    ...
    def cut_keys(self, old_dict):
        '''  
        删除实时表或清洗表中键名末尾的特定后缀(_rd 或 _cd),以便在模拟模块中保持一致性。  

        :param old_dict: 原始字典，包含需要处理的键值对。  
        :return: dict, 一个新字典，其中的键名已删除末尾的后缀。  
        '''  
        new_dict = {}
    ...
    def update_args(self, pending_data):
        '''  
        将从清洗表或实时表中获取的数据与手动输入的数据结合，构造成模拟模块所需的数据格式。  

        :param pending_data: dict, 从清洗表或实时表中获取的数据字典。  
        :return: dict, 模拟模块所需的数据字典，包含更新后的数据。  
        '''  
